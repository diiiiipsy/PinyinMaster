...
for month in ['04', '05', '06', '07', '08', '09', '10', '11']:
    with open ('corpus/sina_news_gbk/2016-' + month + '.txt', 'r', encoding='GBK') as file:
        for line in file:
            ...
            # 获得三元词频
            for i in range(len(html_content) - 2):
                char1 = html_content[i]
                char2 = html_content[i + 1]
                char3 = html_content[i + 2]
                if char1 in char_to_pinyin and char2 in char_to_pinyin and char3 in char_to_pinyin:
                    trigram = f"{char1} {char2} {char3}"
...
trigram_to_words = defaultdict(lambda: {"words": [], "counts": []})
...
for trigram, count in trigram_count.items():
    if count > trigram_limit:
        char1, char2, char3 = trigram.split()
        pinyins1 = char_to_pinyin.get(char1, [])
        pinyins2 = char_to_pinyin.get(char2, [])
        pinyins3 = char_to_pinyin.get(char3, [])
        for pinyin1 in pinyins1:
            for pinyin2 in pinyins2:
                for pinyin3 in pinyins3:
                    pinyin_trigram = f"{pinyin1} {pinyin2} {pinyin3}"
                    trigram_to_words[pinyin_trigram]["words"].append(trigram)
                    trigram_to_words[pinyin_trigram]["counts"].append(count)

output_path = "processed_data/3_word.txt"
with open(output_path, "w", encoding="utf-8") as file:
    json.dump(trigram_to_words, file, ensure_ascii=False, indent=4)
