...
with open('processed_data/3_word.txt', 'r', encoding='utf-8') as file:
    triple_frequency_data = json.load(file)
...
def getAppearanceTriple(word_1, word_2, word_3):
    key = (word_1, word_2, word_3)
    if key in appr_triple_cache:
        return appr_triple_cache[key]
    
    pinyins_1 = getPinyin(word_1)
    pinyins_2 = getPinyin(word_2)
    pinyins_3 = getPinyin(word_3)
    if not pinyins_1 or not pinyins_2 or not pinyins_3:
        result = 0

    total_count = 0
    for pinyin_1 in pinyins_1:
        for pinyin_2 in pinyins_2:
            for pinyin_3 in pinyins_3:
                key = f"{pinyin_1} {pinyin_2} {pinyin_3}"
                if key in triple_frequency_data:
                    words = triple_frequency_data[key]['words']
                    counts = triple_frequency_data[key]['counts']
                    trigram = f"{word_1} {word_2} {word_3}"
                    if trigram in words:
                        word_index = words.index(trigram)
                        total_count += counts[word_index]
    if total_count > 0:
        result = total_count
    else:
    ...
    appr_triple_cache[key] = result
    return result


val_triple_cache = {}

def getValTriple(word_1, word_2, word_3):
    key = (word_1, word_2, word_3)
    if key in val_triple_cache:
        return val_triple_cache[key]
    
    triple_count = getAppearanceTriple(word_1, word_2, word_3)
    double_count = getAppearanceDouble(word_1, word_2)

    if triple_count == 0 or double_count == 0:
        result = big_num
    else:
        result = -math.log(triple_count / double_count)

    val_triple_cache[key] = result
    return result
...
def viterbi(pinyin_sequence):
    ...
    if n > 1:
        second_pinyin = pinyin_sequence[1]
        second_chars = getWord(second_pinyin)
        for char in second_chars:
            min_val = float('inf')
            best_prev_char = None
            for prev_char in dp[0].keys():
                val = dp[0][prev_char] + getValDouble(prev_char, char)
                if val < min_val:
                    min_val = val
    ...
    for i in range(2, n):
        ...
        for current_char in current_chars:
            ...
            for prev_char in previous_chars:
                for pre_prev_char in pre_previous_chars:
                    val = dp[i - 1][prev_char] + getValTriple(pre_prev_char, prev_char, current_char)
                    if val < min_val:
                        min_val = val
                        best_prev_char = prev_char
        ...
        # 只保留最小的 20 个值
        sorted_chars = sorted(current_dp.items(), key=lambda x: x[1])[:search_width]
